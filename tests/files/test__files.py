...
def test__nameext():
    victim = ResolveFilePath(nameext="name.extlast")
    assert victim.NAME == "name"
    ...
    victim = ResolveFilePath(nameext=".extlast")
    assert victim.NAME == ""
    assert victim.EXTLAST == "extlast"
...
def test__ext():
    victim = ResolveFilePath(extlast="extlast")
...
def test__extlast():
    victim = ResolveFilePath(name="name", extlast="extlast")
    assert victim.NAME == "name"
    assert victim.EXTLAST == "extlast"
    assert victim.NAMEEXT == "name.extlast"
    assert victim.DIRPATH == CWD
...
def test__filepath():
    victim = ResolveFilePath(filepath=CWD.joinpath("name.extlast"))
    assert victim.NAME == "name"
    assert victim.EXTLAST == "extlast"
    assert victim.NAMEEXT == "name.extlast"
    assert victim.DIRPATH == CWD
    ...
    victim = ResolveFilePath(name="name2", filepath=CWD.joinpath("name.extlast"))
    ...
    victim = ResolveFilePath(filepath=CWD.joinpath("path1", "name.extlast"))
    assert victim.NAME == "name"
    assert victim.EXTLAST == "extlast"
    assert victim.NAMEEXT == "name.extlast"
